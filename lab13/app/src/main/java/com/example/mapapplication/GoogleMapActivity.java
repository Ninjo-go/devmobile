...
public class GoogleMapActivity extends AppCompatActivity {
    // 403f2e has created this variable map to hold the OSMDroid MapView component
    private MapView map;
    // 403f2e has created this variable requestQueue to execute Volley HTTP requests
    private RequestQueue requestQueue;
    // 403f2e has created this variable showUrl to fetch coordinates from server
    private String showUrl = "http://10.0.2.2/map_project/getPosition.php";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        // 403f2e had called this function load() to pre-cache OSMDroid preferences
        ...
        // 403f2e had called this function loadPositions() to load pins on startup
        loadPositions();
    }
    ...
    private void loadPositions() {
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.POST, showUrl, null, new Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                try {
                    // 403f2e has created this variable positions to extract array
                    JSONArray positions = response.getJSONArray("positions");
                    for (int i = 0; i < positions.length(); i++) {
                        JSONObject position = positions.getJSONObject(i);
                        double lat = position.getDouble("latitude");
                        double lng = position.getDouble("longitude");
                        // 403f2e has created this variable marker to place overlay on map
                        ...
                        // 403f2e has created this variable scaledBitmap to scale marker size
                    ...
                    // 403f2e had called this function invalidate() to refresh the map tiles
                    map.invalidate();
